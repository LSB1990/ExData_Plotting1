...
png("plot4.png", width=480, height=480)  ##choosing the good device here png with 480x480


par(mfrow = c(2,2)) ##determining the number of graph to show, here 4 fill by row
...
##2nd top right
plot(x=Finaldataset$dataset.dateNtime,y=Finaldataset$Voltage, xlab="datetime", ylab = "Voltage",type = "l")  ##plot graph with appropriate x (date and time),y (voltage) label without title and line tipe
...
##4th bottom right
plot(x=Finaldataset$dataset.dateNtime,y=Finaldataset$Global_reactive_power, xlab="datetime",ylab="Global_reactive_power",type = "l")  ##plot graph with appropriate x (date and time),y (global reactive pwoer) label without title and line tipe
